        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                result.add(new Commitment(
                    o.getLong("id"), o.optString("title", "Compromisso"),
                    o.optString("category", "📌 Outro"), o.optString("date", ""),
                    o.optString("time", ""), o.optString("notes", ""),
    ...
    public void saveAll(List<Commitment> items) {
        ...
        for (Commitment item : items) {
            try {
                ...
                o.put("id", item.id); o.put("title", item.title); o.put("category", item.category);
                o.put("date", item.date); o.put("time", item.time); o.put("notes", item.notes);
                o.put("completed", item.completed); array.put(o);
            } catch (Exception ignored) { }
        }
